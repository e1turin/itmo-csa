...
но при этом имеет и свои недостатки такие, к примеру, как множественное обращений
...
адреса.  Для навигации по Page Table используются последние 27 значащих бит VA, 
первые **12** бит используются для адресации внутри физической страницы 
($2^{12}= 4096 = \mathrm{PAGESIZE}$ 🤓). Все последние не используемые биты 64 битного адреса должны иметь значение последнего значащего бита адреса согласно
спецификации. 
...
![xv6-book-vm-concept](./resources/vm-xv6-book-vm-concept.png)

Тут стоит отметить, что для
...
PD для конкретного процесса. Регистр `satp` отвечает именно за это (всё довольно
просто, ничего нового 🧐). В нем хранится режим $\mathrm{MODE}$ в котором работает 
трансляция, PPN и еще кое-что по мелочи.

Иллюстративно это можно представить так (из xv6-book):

![xv6-riscv-address-translation-details](./resources/vm-xv6-riscv-address-translation-details.png)

или так, обращая внимание на то, что Page Table это всего лишь набор физических страниц памяти (моё художество):


![](./resources/vm-riscv-adress-translation-process-my-fig.png)
...
![riscv-adress-translation-process-steps](./resources/vm-riscv-adress-translation-process-steps.png)
...
границе своего размера (элегантное решение ☕).
...
Иллюстрация размещения VA в ядре (из xv6-book):

![xv6-riscv-kernel-space-vm-layout](./resources/vm-xv6-riscv-kernel-space-vm-layout.png)
...
![xv6-riscv-user-process-vm-layout](./resources/vm-xv6-riscv-user-process-vm-layout.png)
...
доступ к памяти» (Direct Memory Access) и Memory-mapped IO. (Случайный доступ к
памяти файла так происходит значительно быстрее, чем через `lseek`, `fseek` 😎)
...
- За гранью Intel и Linux – необычные процессоры и операционные системы –
